...
def define_G(opt):
    ...
    elif which_model == 'modulate_sr_resnet_one':
        netG = arch.ModulateSRResNet_one(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'], nf=opt_net['nf'], nb=opt_net['nb'],
                                     upscale=opt_net['scale'], norm_type=opt_net['norm_type'], mode=opt_net['mode'],
                                     upsample_mode='pixelshuffle', ada_ksize=opt_net['ada_ksize'],
                                     gate_conv_bias=opt_net['gate_conv_bias'])
    elif which_model == 'modulate_sr_resnet_two':
        netG = arch.ModulateSRResNet_two(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'], nf=opt_net['nf'], nb=opt_net['nb'],
                                     upscale=opt_net['scale'], norm_type=opt_net['norm_type'], mode=opt_net['mode'],
                                     upsample_mode='pixelshuffle', ada_ksize=opt_net['ada_ksize'],
                                     gate_conv_bias=opt_net['gate_conv_bias'])

    elif which_model == 'Noise_subnet':
        netG = arch.noise_subnet(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'])

    elif which_model == 'Blur_subnet':
        netG = arch.blur_subnet(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'])
...
def define_sub(opt):
    ...
    if which_model == 'noise_subnet':
        ...
        subnet = arch.noise_subnet(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'])


    elif which_model == 'Blur_subnet':
        subnet = arch.blur_subnet(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'])
