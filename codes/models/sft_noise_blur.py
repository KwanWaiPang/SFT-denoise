...
#            self.real_blur = data['MAT'].to(self.device)
    ...
    def __define_grad_params(self, finetune_type=None):
        ...
        optim_params = []
         # 1. Only the layer whose name includes "Gata" in netG is optimized
        if finetune_type == 'sft':
        	# netG.named_parameters() return the name of param(k), and it's variable(v)
            for k, v in self.netG.named_parameters():
                v.requires_grad = False   # firstly, setting 'not optimize' for all layers
                # only if the name of layer includes 'Gate', optimize
                if k.find('Gate') >= 0:
                    v.requires_grad = True
                    optim_params.append(v)
                    print('we only optimize params: {}'.format(k))
        # 2. the layer whose name includes "Gata" in netG
        #    and the layer whose name includes "degration" in subnet are optimized
         ...
         # 3. Only all the layers in netG are optimized
        ...
        # 4. Optimize all the layers including netG and subnet
        else:
    ...
    def optimize_parameters(self, step):
        ...
        #blur est net 
        self.fake_blur = self.subnet_blur(self.var_L)
        
        self.input_noise_blur = torch.cat((self.fake_noise, self.fake_blur), 1)
        
        self.fake_H = self.netG((self.var_L,self.input_noise_blur))
